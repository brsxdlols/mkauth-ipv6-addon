...
function cgnatBuildPdf($result, $o, $mappingRows=null, $generatedAt=null)
{
    $rows = is_array($mappingRows) ? $mappingRows : cgnatMappingRows($result); $perPage = 38;
    ...
    foreach ($pages as $pageIndex=>$pageRows) {
        ...
        $pdfDate=$generatedAt ? date('d/m/Y H:i',strtotime($generatedAt)) : date('d/m/Y H:i');
        $subtitle='CGNAT-'.strtoupper(trim(!empty($o['name'])?$o['name']:'001')).' | '.$result['private_cidr'].' > '.$result['public_cidr'].' | '.$pdfDate;
